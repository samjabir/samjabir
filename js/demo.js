 ...
 */
{
	  ...
	  spread() {
		...
		this.DOM.items.forEach(item => {
		  ...
		  const randnum = getRandom(
			this.config.titltOffset.min,
			this.config.titltOffset.max
		  );
		  ...
		  const margins = { x: winsize.width * 0.05, y: winsize.height * 0.05 };
		  const tx =
			quadrant === 1 || quadrant === 4
			  ? getRandom(
				  -1 * center.x + winsize.width / 2 + margins.x,
				  winsize.width - center.x - margins.x
				)
			  : getRandom(
				  -1 * center.x + margins.x,
				  winsize.width / 2 - center.x - margins.x
				);
		  const ty =
			quadrant === 1 || quadrant === 2
			  ? getRandom(
				  -1 * center.y + margins.y,
				  winsize.height / 2 - center.y - margins.y
				)
			  : getRandom(
				  -1 * center.y + winsize.height / 2 + margins.y,
				  winsize.height - center.y - margins.y
				);
	  ...
	  toggleItems(action, direction) {
		return new Promise((resolve, reject) => {
		  ...
		  this.DOM.items.forEach(item => {
			...
			TweenMax.to(item, action === "show" ? speed * 0.55 : speed * 0.45, {
			  ...
			  onComplete: () => {
				TweenMax.to(
				  item,
				  action === "show" ? speed * 0.45 : speed * 0.55,
				  {
	...
	class MenuItem {
	  ...
	  toggleCurrent(direction = "up") {
		...
		this.DOM.el.classList[isCurrent ? "remove" : "add"](
		  "menu__item--current"
		);
		...
		TweenMax.to(this.DOM.link, 1, {
		  ...
		  startAt: isCurrent
			? null
			: { opacity: 0, y: direction === "up" ? 15 : -15 },
	  ...
	  toggle(action) {
		...
		if (
		  action === "show" &&
		  !this.DOM.el.classList.contains("menu__item--current")
		) {
	...
	class ContentItem {
	  ...
	  toggleCurrent() {
		this.DOM.el.classList[
		  this.DOM.el.classList.contains("content__item--current")
			? "remove"
			: "add"
	...
	class NavController {
	  constructor(el) {
		...
		this.menuItems = [];
		Array.from(this.DOM.menu.querySelectorAll(".menu__item")).forEach(item =>
		...
		this.contentItems = [];
		Array.from(
		  this.DOM.page.querySelectorAll(".content > .content__item")
		).forEach(item => this.contentItems.push(new ContentItem(item)));
		// The grid instances.
		this.grids = [];
		Array.from(this.DOM.gridWrap.querySelectorAll(".grid")).forEach(grid =>
	  ...
	  init() {
		...
		this.grids[this.current].DOM.items.forEach(item =>
	  ...
	  initEvents() {
		...
		this.mousemoveFn = ev =>
		  requestAnimationFrame(() => this.grids[this.current].tilt(ev));
	  ...
	  showContent(pos) {
		if (
		  this.isAnimating ||
		  this.grids[this.current].DOM.el.classList.contains("grid--open")
		)
		  return;
		this.isAnimating = true;
		// Disable the mousemove functionality.
		...
		this.DOM.menu.classList.add("d--none");
		// Show the grid.
	  ...
	  hideContent() {
		...
		scrollIt(0, 300, "easeOutQuad", () => {
		  ...
		  this.grids[this.current].close().then(() => {
			...
			this.DOM.menu.classList.remove("d--none");
